...
namespace TRUEStudios.Events {
	public abstract class EventListenerBase<T0, T1, T2> : MonoBehaviour {
		public abstract void OnInvoke (T0 v0, T1 v1, T2 v2);
	}
	
	public abstract class EventListener<TUnityEvent, TEventHub, T0, T1, T2> : EventListenerBase<T0, T1, T2>
		where TUnityEvent : UnityEvent<T0, T1, T2>, new()
		where TEventHub : EventHub<T0, T1, T2>, new() {
		...
		public override void OnInvoke (T0 v0, T1 v1, T2 v2) {
			_onResponse.Invoke(v0, v1, v2);
		}
		#endregion
	}
}
