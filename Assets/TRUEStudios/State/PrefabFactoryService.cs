...
namespace TRUEStudios.State
{
		...
		where TBase : MonoBehaviour
	{
		...
		protected T PushInstance<T>(string prefabName, Transform spawnTransform)
			where T : TBase
		{
			PrefabProfile profile = null;
		...
		protected T PushInstance<T>(GameObject prefab, Transform spawnTransform)
			where T : TBase
		{
			T component = null;
		...
		public PrefabProfile GetPrefabProfileByName<T>(string prefabName)
			where T : TBase
		{
			// check if the profile already exists
			PrefabProfile profile = null;
